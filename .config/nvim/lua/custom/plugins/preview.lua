-- Generic "toggle preview" dispatcher: <leader>tp calls whichever
-- filetype-specific previewer owns the current buffer's filetype. To add a
-- new one, register a function into `previewers[filetype]` -- either here
-- (if it's a lazy.nvim plugin's `config`, like markdown below) or from any
-- other custom/settings module (like graphviz.lua's `dot` previewer) -- no
-- separate keymap needed either way.
--
-- `_G.previewers` (not a plain local) because custom/settings/graphviz.lua
-- registers into this same table but loads earlier, directly from
-- init.lua, than this file (loaded later via lazy.nvim's `custom.plugins`
-- import) -- `or {}` picks up whatever it already added instead of
-- clobbering it.
_G.previewers = _G.previewers or {}
local previewers = _G.previewers
