...
describe('permalink.build_url', function()
  ...
  it('builds a github permalink with a line range', function()
    local url = permalink.build_url('git@github.com:ashvinsharma/dotfiles.git', 'abc123', '.config/nvim/init.lua', { 10, 20 })
    assert.equals('https://github.com/ashvinsharma/dotfiles/blob/abc123/.config/nvim/init.lua#L10-L20', url)
  end)

  it('builds a gitlab permalink with a line range', function()
    local url = permalink.build_url('git@gitlab.com:my-group/my-project.git', 'def456', 'lib/foo.rb', { 10, 20 })
    assert.equals('https://gitlab.com/my-group/my-project/-/blob/def456/lib/foo.rb#L10-20', url)
  end)

  it('collapses a single-line range to a plain #L anchor', function()
    local url = permalink.build_url('git@gitlab.com:my-group/my-project.git', 'def456', 'lib/foo.rb', { 5, 5 })
    assert.equals('https://gitlab.com/my-group/my-project/-/blob/def456/lib/foo.rb#L5', url)
  end)

  it('normalizes a reversed range (end line before start line)', function()
    local url = permalink.build_url('git@gitlab.com:my-group/my-project.git', 'def456', 'lib/foo.rb', { 20, 10 })
    assert.equals('https://gitlab.com/my-group/my-project/-/blob/def456/lib/foo.rb#L10-20', url)
  end)
end)
